...
def sequential_coordinate_search(fitness_func, alp, P_exp, az_m, ax_m, ay_m,
    ...
    for p in range(n_passes):
        print(f"--- координатный поиск: проход {p+1}/{n_passes} ---")
        ...
        history[f"pass{p+1}_tau"] = (tau_vals_full, fit_tau)
...
if __name__ == "__main__":
    ...
    print("\n=== Sequential coordinate search (tau -> Kz -> Kx -> Ky, curve_fit fitness) ===")
    t0 = time.perf_counter()
    tau_sq, Kz_sq, Kx_sq, Ky_sq, fit_sq, calls_sq, hist_sq = sequential_coordinate_search(
        curvefit_fitness, alp, P_sim_noise, az_m, ax_m, ay_m,
        tau_range, Kz_range, Kx_range, Ky_range,
        n_Kz=401, n_Kx=201, n_Ky=201, n_passes=2,
        tau_init=500, Kz_init=Kz_nominal, Kx_init=Kx_nominal, Ky_init=Ky_nominal)
    results["Sequential (curve_fit)"] = (tau_sq, Kz_sq, Kx_sq, Ky_sq, time.perf_counter() - t0, calls_sq)
    ...
    # --- ход координатного поиска (последний проход) ---
    fig, axes = plt.subplots(1, 4, figsize=(16, 4))
    last_pass = 2
    for ax, name, opt_val in zip(
            axes, ["tau", "Kz", "Kx", "Ky"], [tau_sq, Kz_sq, Kx_sq, Ky_sq]):
        xv, fv = hist_sq[f"pass{last_pass}_{name}"]
        ax.plot(xv, fv)
        ax.axvline(opt_val, color="k", ls="--", label=f"{name}_opt={opt_val:.4g}")
        ax.set_xlabel(name)
        ax.set_ylabel("fitness")
        ax.set_title(f"Координатный поиск: {name} (проход {last_pass})")
        ax.legend()
    fig.tight_layout()
